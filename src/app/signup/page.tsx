    ...
    if (hasFirebaseConfig && auth) {
      try {
        ...
        if (methods.length > 0) {
          if (methods.includes("google.com")) {
            error(
              ...
              "This email is already registered with Google Sign-In. Please use Google to log in."
            );
            setLoading(false);
            return;
          }
          error("Account exists", "An account with this email already exists. Please log in instead.");
          setLoading(false);
          return;
        }
        ...
        if (err.code === "auth/email-already-in-use") {
          error("Email in use", "This email is already registered. Try logging in.");
        } else {
          error("Signup failed", err.message || "Something went wrong.");
        }
      }
    } else {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (err.code === "auth/account-exists-with-different-credential") {
        error(
          "Account exists",
          "An account with this email already exists using email/password. Please log in with your password first, then link Google."
        );
