  ...
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="background: white; padding: 30px; border: 1px solid #e5e7eb; border-top: none;">
      ...
      <p style="font-size: 14px; color: #555; margin: 0 0 25px; line-height: 1.6;">
        Thank you for shopping with us! Your order has been placed successfully. Here's your invoice:
      </p>
      ...
      <div style="background: #f9fafb; border-radius: 8px; padding: 16px; margin-bottom: 25px;">
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="font-size: 12px; color: #666; padding: 4px 0;">Order ID</td>
            <td style="font-size: 12px; color: #333; padding: 4px 0; text-align: right; font-weight: 600;">#${data.orderId.slice(0, 8).toUpperCase()}</td>
          </tr>
          <tr>
            <td style="font-size: 12px; color: #666; padding: 4px 0;">Date</td>
            <td style="font-size: 12px; color: #333; padding: 4px 0; text-align: right;">${orderDate}</td>
          </tr>
          <tr>
            <td style="font-size: 12px; color: #666; padding: 4px 0;">Payment</td>
            <td style="font-size: 12px; color: #333; padding: 4px 0; text-align: right; text-transform: uppercase;">${data.paymentMethod}</td>
          </tr>
        </table>
      </div>
      ...
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
        <thead>
          <tr style="background: #f9fafb;">
            ...
            <th style="padding: 10px 12px; text-align: right; font-size: 12px; color: #666; font-weight: 600;">Price</th>
      ...
      <div style="border-top: 2px solid #c2410c; padding-top: 15px; text-align: right;">
        <p style="font-size: 18px; color: #c2410c; margin: 0; font-weight: 700;">
    ...
    <div style="text-align: center; padding: 20px;">
      <p style="font-size: 11px; color: #999; margin: 0;">© 2025 TheIdeaDecorator. All rights reserved.</p>
    </div>
  </div>
</body>
</html>`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!smtpHost || !smtpUser || !smtpPass) {
      // If SMTP not configured, just return success without sending
      // (useful for development)
      console.log("SMTP not configured — skipping email send for order:", data.orderId);
      return NextResponse.json({ 
        success: true, 
        message: "Order confirmed (email skipped — SMTP not configured)",
    ...
    await transporter.sendMail({
      from: `"TheIdeaDecorator" <${fromEmail}>`,
